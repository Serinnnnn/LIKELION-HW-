...
# 세번째 인자---앞에있는 key 값--html 에서 어떤 변수명으로 받을지 
...
# render = 그리다 , html 가지고 화면을 그려내는 것
# redirect --> 이동시켜 url만 바꿔주는 것, GET요청은 render, POST, DELETE 등은 redirect 
...
# update는 queryset class의 method 이기 때문에 이를  활용하려면 filter를 사용해서 queryset을 조회해야함
# {'post': post[0]}) 리스트를 반환했으니까 0번째 포스트만 꺼내서 넘겨주고 있는 모습
